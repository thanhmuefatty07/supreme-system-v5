...
class MetricPoint:
    ...
    def _generate_checksum(self) -> str:
        ...
        data = f"{self.timestamp.isoformat()}{self.value}{json.dumps(self.metadata or {}, sort_keys=True)}"
...
class SecureDashboard:
    ...
    def __init__(self, max_metrics: int = 10000, rate_limit: int = 100):
        ...
        self.request_tracker = deque(maxlen=rate_limit)
    ...
    def _check_rate_limit(self) -> bool:
        ...
        if len(self.request_tracker) >= self.rate_limit:
            logger.warning(f"Rate limit exceeded: {len(self.request_tracker)} requests")
            return False
    ...
    def record_metric(self, metric_name: str, value: float, metadata: Dict[str, Any] = None) -> bool:
        """
        ...
        try:
            ...
            # Input sanitization
            metric_name = self._sanitize_metric_name(metric_name)
            if not metric_name:
            ...
            logger.error(f"Unexpected error in record_metric: {e}")
            return False
    ...
    async def get_dashboard_data(self, session_token: str = None, minutes: int = 10) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Input validation
            minutes = max(1, min(1440, int(minutes)))  # 1 minute to 24 hours
            ...
            with self._lock:
                dashboard_data = {
                    ...
                    'metrics': self._get_secure_metrics(minutes),
                    'system_info': {
    ...
    def _get_secure_metrics(self, minutes: int) -> Dict[str, Any]:
        ...
        for metric_name, points in self.metrics.items():
            try:
                ...
                recent_points = [
                    p for p in points 
                    if p.timestamp > cutoff_time and p.verify_integrity()
                ]
                ...
                secure_metrics[metric_name] = {
                    ...
                    'points': [p.to_dict() for p in recent_points[-50:]]  # Limit to last 50 points
                }
...
'''
                ...
                for metric_name, metric_data in list(metrics.items())[:10]:  # Limit to 10 metrics
...
async def generate_secure_dashboard_html(session_token: str = None) -> str:
...
if __name__ == "__main__":
    ...
    async def secure_demo():
        ...
        print("🔒 Security features: CSP, Rate limiting, Input validation, CSRF protection")
