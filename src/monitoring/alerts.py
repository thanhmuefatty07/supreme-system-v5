...
class Alert:
    ...
    def is_suppressed(self) -> bool:
        ...
        return (
            ...
            and datetime.utcnow() < self.suppressed_until
        )
...
class AlertManager:
    ...
    def __init__(self) -> None:
        ...
        logger.info("   Configured thresholds: %d metrics", len(self.thresholds))
    ...
    def update_metric_value(self, metric_name: str, value: float) -> None:
        ...
        self.metric_values[metric_name].append({"timestamp": timestamp, "value": value})
    ...
    def _check_threshold(self, threshold: AlertThreshold, current_value: float, timestamp: float) -> None:
    ...
    def _handle_threshold_breach(
        self, threshold: AlertThreshold, value: float, timestamp: float, alert_key: str
    ) -> None:
    ...
    def _handle_threshold_recovery(
        self, threshold: AlertThreshold, value: float, alert_key: str
    ) -> None:
    ...
    async def _send_alert_notification(self, alert: Alert) -> None:
        ...
        for callback in self.notification_callbacks:
            try:
                ...
                logger.error("Alert notification callback failed: %s", exc)
    ...
    async def _send_recovery_notification(self, alert: Alert) -> None:
        ...
        for callback in self.notification_callbacks:
            try:
                ...
                logger.error("Recovery notification callback failed: %s", exc)

        logger.info("✅ RECOVERED: %s alert recovered", alert.threshold.metric_name)
    ...
    def suppress_alert(
        self, metric_name: str, severity: AlertSeverity, duration_minutes: float = 5.0
    ) -> None:
        ...
        if alert_key in self.active_alerts:
            ...
            logger.info("🔇 Alert suppressed: %s for %s minutes", alert_key, duration_minutes)
...
def configure_alert_thresholds() -> AlertManager:
    ...
    logger.info("✅ Alert thresholds configured: %d metrics", len(alert_manager.thresholds))
...
async def check_metric_alerts(metric_name: str, value: float) -> None:
    """Check metric against configured alert thresholds"""
    alert_manager.update_metric_value(metric_name, value)


async def broadcast_alert_to_websocket(alert_data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.error("Failed to broadcast alert to WebSocket: %s", exc)
