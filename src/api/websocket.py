...
class ClientConnection:
    ...
    def can_receive_message(self, priority: MessagePriority) -> bool:
        ...
        if priority == MessagePriority.HIGH and queue_ratio < 0.8:
            return True
        if priority == MessagePriority.NORMAL and queue_ratio < 0.6:
            return True
        if priority == MessagePriority.LOW and queue_ratio < 0.4:
            return True
    ...
    def add_message(self, message: WebSocketMessage) -> bool:
        ...
        if not self.can_receive_message(message.priority):
            ...
            logger.debug(
                "Dropped message for client %s due to backpressure", self.client_id
            )
            return False
...
class WebSocketHandler:
    ...
    def __init__(
        self,
        freq_config: Optional[MessageFrequencyConfig] = None,
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "   Message frequencies: High=%dms, Medium=%dms",
            self.freq_config.high_frequency_ms,
            self.freq_config.medium_frequency_ms,
        )
        logger.info(
            "   Backpressure threshold: %s", self.backpressure_config.drop_threshold
        )
    ...
    async def connect_client(self, websocket: WebSocket, client_id: Optional[str] = None) -> str:
        ...
        logger.info("🔌 Client %s connected (anonymous). Total: %d", client_id, len(self.connections))
        return client_id
    ...
    async def disconnect_client(self, client_id: str) -> None:
        ...
        logger.info("   Messages sent: %d", client.message_count)
        logger.info("   Messages dropped: %d", client.dropped_messages)
        logger.info("   Connection quality: %.2f", client.connection_quality)

        del self.connections[client_id]

        logger.info("🔌 Client disconnected. Remaining: %d", len(self.connections))
    ...
    async def handle_client_message(self, client_id: str, message: str) -> None:
        ...
        try:
            ...
            logger.error("Invalid JSON from client %s: %s", client_id, message)
        except Exception as exc:
            logger.error("Error handling message from %s: %s", client_id, exc)

    async def broadcast_message(
        self, message_type: MessageType, data: Dict[str, Any], target_clients: Optional[Set[str]] = None
    ) -> None:
        ...
        if eligible_clients:
            tasks = [self._send_to_client_queued(client_id, message) for client_id in eligible_clients]
            await asyncio.gather(*tasks, return_exceptions=True)

    async def _send_to_client_queued(self, client_id: str, message: WebSocketMessage) -> None:
    ...
    async def _send_to_client_immediately(self, client_id: str, message: WebSocketMessage) -> None:
    ...
    async def _handle_subscription(self, client_id: str, subscriptions: List[str]) -> None:
        """Handle subscription changes"""
        if client_id in self.connections:
            client = self.connections[client_id]
            try:
                client.subscriptions = {MessageType(sub) for sub in subscriptions}
                logger.info("🔔 Client %s updated subscriptions: %s", client_id, subscriptions)
            except ValueError as exc:
                logger.warning("Invalid subscription from %s: %s", client_id, exc)

    async def _handle_unsubscription(self, client_id: str, unsubscriptions: List[str]) -> None:
        ...
        if client_id in self.connections:
            ...
            try:
                ...
                logger.info("🔕 Client %s unsubscribed from: %s", client_id, unsubscriptions)
            except ValueError as exc:
                logger.warning("Invalid unsubscription from %s: %s", client_id, exc)
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        total_dropped = sum(
            client.dropped_messages for client in self.connections.values()
        )
        avg_quality = (
            sum(client.connection_quality for client in self.connections.values()) / len(self.connections)
            if self.connections
            else 0
        )
...
websocket_handler = WebSocketHandler(
    ...
    BackpressureConfig(max_queue_size=1000, drop_threshold=0.8, priority_preservation=True),
)


# Convenience functions for easy broadcasting of the 6 message types
async def broadcast_performance(performance_data: Dict[str, Any]) -> None:
    """Broadcast performance data (high frequency)"""
    await websocket_handler.broadcast_message(MessageType.PERFORMANCE, performance_data)


async def broadcast_trading_status(trading_data: Dict[str, Any]) -> None:
    """Broadcast trading status (medium frequency)"""
    await websocket_handler.broadcast_message(MessageType.TRADING_STATUS, trading_data)


async def broadcast_portfolio_update(portfolio_data: Dict[str, Any]) -> None:
    """Broadcast portfolio update (medium frequency)"""
    await websocket_handler.broadcast_message(MessageType.PORTFOLIO_UPDATE, portfolio_data)


async def broadcast_order_update(order_data: Dict[str, Any]) -> None:
    """Broadcast order update (immediate)"""
    await websocket_handler.broadcast_message(MessageType.ORDER_UPDATE, order_data)


async def broadcast_system_alert(message: str, severity: str = "info", **kwargs: Any) -> None:
    """Broadcast system alert (immediate)"""
    alert_data = {"message": message, "severity": severity, **kwargs}
    await websocket_handler.broadcast_message(MessageType.SYSTEM_ALERT, alert_data)


async def broadcast_component_status(component_data: Dict[str, Any]) -> None:
    """Broadcast component status (low frequency)"""
    await websocket_handler.broadcast_message(MessageType.COMPONENT_STATUS, component_data)
...
async def handle_websocket_connection(websocket: WebSocket, client_id: Optional[str] = None) -> None:
    ...
    try:
        ...
        logger.error("WebSocket connection error for %s: %s", client_id, exc)
        await websocket_handler.disconnect_client(client_id)
