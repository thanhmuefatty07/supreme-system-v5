            ...
            for pattern, replacement_template in patterns:
                ...
                if modified_content != old_content:
                    ...
                    count = (
                        old_content.count('f"')
                        + old_content.count("f'")
                        - (modified_content.count('f"') + modified_content.count("f'"))
                    )
                    if count > 0:
...
Examples:
  ...
  python fix_f541_comprehensive.py phase2_main.py -v  # Fix specific file with verbose
    ...
    parser.add_argument(
        "--live", action="store_true", help="Apply changes (default is dry-run)"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument(
        "--all-py", action="store_true", help="Fix all .py files in current directory"
    )
