...
class OKXPublicConnector:
    ...
    async def subscribe_orderbook(
        self, symbol: str, callback: Callable, depth: str = "5"
    ):
        """
            ...
            async for message in self.websocket:
                try:
                    ...
                    if data.get("event") == "subscribe":
                        logger.info(
                            f"✅ OKX subscription confirmed: {data.get('arg', {})}"
                        )
                        continue
                    ...
                    if "data" in data and "arg" in data:
                        ...
                        if callback_key in self.subscriptions:
                            ...
                            if channel == "tickers":
                                ...
                                await self._process_orderbook_data(
                                    symbol, data["data"][0]
                                )
    ...
    async def _process_ticker_data(self, symbol: str, ticker_data: Dict):
        ...
        try:
            ...
            logger.debug(
                f"📈 OKX {symbol}: ${normalized_data['price']:.4f} vol={normalized_data['volume_24h']:.0f}"
            )
    ...
    async def _process_orderbook_data(self, symbol: str, book_data: Dict):
        ...
        try:
            ...
            if bids and asks:
                ...
                logger.debug(
                    f"📊 OKX {symbol} spread: {spread:.4f} ({spread_bps:.1f} bps)"
                )
