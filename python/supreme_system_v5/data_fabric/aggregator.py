...
class DataSource:
    ...
    def record_error(self, error_type: str):
        ...
        if self.error_count >= self.max_errors:
            ...
            logger.error(f"🚨 Circuit breaker opened for {self.name} ({self.error_count} errors)")
...
class DataAggregator:
    ...
    async def get_market_data(self, symbol: str, max_age_seconds: float = 60) -> Optional[MarketDataPoint]:
        """
        ...
        for source in available_sources[:3]:  # Try top 3 sources
            try:
                ...
                if raw_data:
                    ...
                    DATA_QUALITY.labels(source=source.name, symbol=symbol).set(quality.overall_score)
                    
                    data_points.append((normalized, quality, source.weight))
                    logger.debug(f"✅ {source.name}: {symbol} = ${normalized.price:.4f} (quality: {quality.overall_score:.2f})")
    ...
    def _aggregate_data_points(self, symbol: str, data_points: List[tuple]) -> MarketDataPoint:
        """
    ...
    async def _stream_symbol_data(self, symbol: str, interval_seconds: float):
        ...
        while self.running:
            try:
                ...
                data = await self.get_market_data(symbol, max_age_seconds=interval_seconds)
    ...
    async def _monitor_source_health(self):
        ...
        while self.running:
            ...
            if health_ratio < 0.5:
                logger.error(f"🚨 Data fabric degraded: {healthy_sources}/{total_sources} sources healthy")
            elif health_ratio < 0.8:
                logger.warning(f"⚠️ Data fabric partial: {healthy_sources}/{total_sources} sources healthy")
