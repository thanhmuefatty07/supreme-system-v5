...
RISK_VIOLATIONS = Counter('risk_violations_total', 'Risk violations by type', ['violation_type'])
...
class RiskManager:
    ...
    def __init__(self, limits: Optional[RiskLimits] = None, portfolio_state: Optional[PortfolioState] = None):
    ...
    def _calculate_drawdown(self) -> float:
        ...
        drawdown = (self.portfolio.peak_value - current_value) / self.portfolio.peak_value * 100
        return drawdown
    ...
    def _calculate_daily_loss_percent(self) -> float:
        ...
        loss_percent = (self.portfolio.daily_start_value - self.portfolio.total_value) / self.portfolio.daily_start_value * 100
        return loss_percent
    ...
    def _check_portfolio_limits(self, trade_value: float) -> RiskAssessment:
        ...
        if self.limits.cool_off_period_minutes > 0:
            cool_off_until = self.portfolio.last_trade_time + (self.limits.cool_off_period_minutes * 60)
            if time.time() < cool_off_until:
                remaining_minutes = (cool_off_until - time.time()) / 60
                assessment.warnings.append(f"Cool-off period active: {remaining_minutes:.1f} minutes remaining")
    ...
    def _check_position_limits(self, symbol: str, position_value: float, leverage: float = 1.0) -> RiskAssessment:
        ...
        if portfolio_percent > self.limits.max_position_size_percent:
            ...
            # Suggest adjusted size
            max_portfolio_size = self.portfolio.total_value * (self.limits.max_position_size_percent / 100)
            if assessment.adjusted_position_size is None or max_portfolio_size < assessment.adjusted_position_size:
                assessment.adjusted_position_size = max_portfolio_size

        # Check leverage limit
        if leverage > self.limits.max_leverage:
            assessment.add_violation(
                RiskViolation.LEVERAGE_LIMIT,
                ".1f"
            )
    ...
    def evaluate_trade(self, symbol: str, position_value: float, leverage: float = 1.0) -> RiskAssessment:
        """
        ...
        position_assessment = self._check_position_limits(symbol, position_value, leverage)
        ...
        if portfolio_assessment.adjusted_position_size is not None or position_assessment.adjusted_position_size is not None:
        ...
        if final_assessment.approved:
            ...
            final_assessment.reasoning = f"❌ Trade rejected - {len(final_assessment.violations)} risk violations"
            ...
            if len(final_assessment.violations) > 0:
                ...
                if self.portfolio.violation_count >= self.limits.circuit_breaker_threshold:
                    self.portfolio.circuit_breaker_active = True
                    self.portfolio.circuit_breaker_until = time.time() + (self.limits.circuit_breaker_timeout_minutes * 60)

                    logger.error(f"🚨 Circuit breaker activated! {self.portfolio.violation_count} violations in sequence")
                    logger.error(f"⏰ Circuit breaker active until {datetime.fromtimestamp(self.portfolio.circuit_breaker_until)}")
    ...
    def record_trade(self, symbol: str, side: str, quantity: float, price: float, pnl: float = 0.0):
        ...
        self.portfolio.total_value = self.portfolio.cash_balance + sum(self.portfolio.position_values.values())
        self.portfolio.peak_value = max(self.portfolio.peak_value, self.portfolio.total_value)
        ...
        logger.info(f"📊 Portfolio value: ${self.portfolio.total_value:.2f} (PnL: ${self.portfolio.total_pnl:.2f})")
    ...
    def reset_portfolio(self, new_value: float = 10000.0):
        ...
        """
        self.portfolio = PortfolioState(total_value=new_value, cash_balance=new_value, peak_value=new_value, daily_start_value=new_value)
        self.last_daily_reset = self._get_day_start()
    ...
    def get_adaptive_limits(self) -> RiskLimits:
        ...
        if current_drawdown > 5.0:  # Over 5% drawdown
            drawdown_factor = max(0.5, 1.0 - (current_drawdown / self.limits.max_drawdown_percent))
            adaptive_limits.max_position_size_percent *= drawdown_factor
            adaptive_limits.max_daily_loss_percent *= drawdown_factor
            logger.info(f"🔴 High drawdown detected: {current_drawdown:.2f}%, reducing limits by {drawdown_factor:.2f}x")
        # Increase limits if win rate is high and drawdown is low
        elif win_rate > 0.6 and current_drawdown < 2.0 and self.portfolio.total_trades > 10:
            confidence_factor = min(1.2, 1.0 + (win_rate - 0.5))
            adaptive_limits.max_position_size_percent *= confidence_factor
            logger.info(f"🟢 Strong performance detected: {win_rate:.1%} win rate, increasing limits by {confidence_factor:.2f}x")
