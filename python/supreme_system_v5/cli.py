...
from .exchanges import BINANCE_AVAILABLE, OKX_AVAILABLE
...
def status():
    ...
    table.add_row(
        "Python Version", "✅ OK", f"{sys.version_info.major}.{sys.version_info.minor}"
    )
    table.add_row(
        "Rust Engine",
        "✅ Available" if RUST_ENGINE_AVAILABLE else "⚠️ Fallback",
        "PyO3 integration",
    )
    table.add_row(
        "OKX Connector",
        "✅ Available" if OKX_AVAILABLE else "❌ Missing",
        "Primary exchange",
    )
    table.add_row(
        "Binance Connector",
        "✅ Available" if BINANCE_AVAILABLE else "❌ Missing",
        "Backup exchange",
    )
